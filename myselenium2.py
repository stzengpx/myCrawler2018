...
class myselenium2():
    def __init__(self):
        ...
        self.tableMylog = [
            ['', '', '', '', '']
        ]  # ['DateTime', 'qryCond', 'DataRowsTotal', 'PageTotal', 'PageCurrent']
        self.tableMyError = []
    ...
    def mySendMailUsage(self, _issue):
        try:
            ...
            msg['Subject'] = 'myCrawler2018_user_' + self.serial_number.decode(
                "utf-8"
            ) + '_' + self.myQryCondMailTitleCity + '_' + self.myQryCondMailTitleArea + '_' + _issue
            msg['From'] = gmail_user
            ...
            print(
                self.serial_number.decode("utf-8") +
                ' - Login alert was sent.')
    ...
    def MyMoreLinkCollection(self, _workbook, _worksheet, _myweb,
                             _PageCurrent):
        CountMoreLinkMouseOut = len(
            _myweb.find_elements_by_xpath("//*[@class='moreLinkMouseOut']"))
        # ListMoreLink = list()
        TmpStockStatus = ""
        TmpAmountReal = ""
        TmpPageTitleTest = ""
        ...
        while i < CountMoreLinkMouseOut + 1:
            ...
            try:
                ...
                if self.isDebugMode:
                    print("[頁,筆]: " + _PageCurrent + "," + str(i) + "," +
                          str(DateTimeNow))
                # Scroll to Element
                # python - Scrolling to element using webdriver? - Stack Overflow - https://goo.gl/1Ci6uf
                element = _myweb.find_element_by_xpath(
                    "(//*[@class='moreLinkMouseOut'])[" + str(i) + "]")
                _myweb.execute_script("arguments[0].scrollIntoView();",
                                      element)
                element.click()
                _myweb.implicitly_wait(self.timeoutImplicitlyWait)
                ...
                if TmpPageTitleTest == "錯誤":
                    ...
                    _myweb.implicitly_wait(self.timeoutImplicitlyWait)
                    continue
                TmpDataType = _myweb.find_element_by_xpath(
                    "//*[@class='tab-content']/div/h3").get_attribute(
                        "innerHTML")
                TmpDataType = TmpDataType.strip()
                # print(TmpDataType)
                if TmpDataType == "工廠基本資料":
                    TmpCorp = _myweb.find_element_by_xpath(
                        "(//table[@class='table table-striped']/tbody/tr/td)[4]"
                    ).get_attribute("innerHTML")
                    TmpAdd = _myweb.find_element_by_xpath(
                        "(//table[@class='table table-striped']/tbody/tr/td)[14]"
                    ).get_attribute("innerHTML")
                    TmpName = _myweb.find_element_by_xpath(
                        "(//table[@class='table table-striped']/tbody/tr/td)[16]"
                    ).get_attribute("innerHTML")
                    ...
                    # print("2")
                    TmpPartnerHead = _myweb.find_element_by_xpath(
                        "(//table[@class='table table-striped']/tbody/tr/td)[13]"
                    ).get_attribute("innerHTML")
                    TmpPartnerHead = TmpPartnerHead.strip()
                    if TmpPartnerHead == "合夥人姓名":
                        TmpCorp = _myweb.find_element_by_xpath(
                            "(//table[@class='table table-striped']/tbody/tr/td)[10]"
                        ).get_attribute("innerHTML")
                        TmpAdd = _myweb.find_element_by_xpath(
                            "(//table[@class='table table-striped']/tbody/tr/td)[22]"
                        ).get_attribute("innerHTML")
                        # TmpName     = _myweb.find_element_by_xpath("(//table[@class='table table-striped']/tbody/tr/td)[12]").get_attribute("innerHTML")
                        TmpName = _myweb.find_element_by_xpath(
                            "//*[@id='tabBusmContent']/div/table/tbody/tr[6]/td[2]/table/tbody/tr/td[1]"
                        ).get_attribute("innerHTML")
                        TmpCorp = TmpCorp[0:TmpCorp.index("<span")].strip()
                        TmpAdd = TmpAdd[0:TmpAdd.index('<span')].strip()
                        TmpName = TmpName.strip()
                        ...
                        TmpAdd = _myweb.find_element_by_xpath(
                            "(//table[@class='table table-striped']/tbody/tr/td)[20]"
                        ).get_attribute("innerHTML")
                        # TmpName     = _myweb.find_element_by_xpath("(//table[@class='table table-striped']/tbody/tr/td)[12]").get_attribute("innerHTML")
                        TmpName = _myweb.find_element_by_xpath(
                            "//*[@id='tabBusmContent']/div/table/tbody/tr[6]/td[2]/table/tbody/tr/td[1]"
                    ...
                    # print("3")
                    TmpCorp = _myweb.find_element_by_xpath(
                        "(//table[@class='table table-striped']/tbody/tr/td)[18]"
                    ).get_attribute("innerHTML")
                    TmpAdd = _myweb.find_element_by_xpath(
                        "(//table[@class='table table-striped']/tbody/tr/td)[10]"
                    ).get_attribute("innerHTML")
                    TmpName = _myweb.find_element_by_xpath(
                        "(//table[@class='table table-striped']/tbody/tr/td)[8]"
                    ).get_attribute("innerHTML")
                    TmpCorp = TmpCorp[TmpCorp.index('">') +
                                      2:TmpCorp.index("</a>")].strip()
                    TmpAdd = TmpAdd[0:TmpAdd.index('<span')].strip()
                    TmpName = TmpName.strip()
                elif TmpDataType == "外國公司登記基本資料":
                    TmpCorp = _myweb.find_element_by_xpath(
                        "(//table[@class='table table-striped']/tbody/tr/td)[6]"
                    ).get_attribute("innerHTML")
                    TmpAdd = _myweb.find_element_by_xpath(
                        "(//table[@class='table table-striped']/tbody/tr/td)[12]"
                    ).get_attribute("innerHTML")
                    TmpName = _myweb.find_element_by_xpath(
                        "(//table[@class='table table-striped']/tbody/tr/td)[10]"
                    ).get_attribute("innerHTML")
                    ...
                    TmpCorpHeader = _myweb.find_element_by_xpath(
                    ...
                    # print("5")
                    TmpStockStatus = _myweb.find_element_by_xpath(
                        "(//table[@class='table table-striped']/tbody/tr/td)[6]"
                    ).get_attribute("innerHTML")
                    TmpStockStatus = TmpStockStatus.strip()
                    TmpCorpType = _myweb.find_element_by_xpath(
                        "(//table[@class='table table-striped']/tbody/tr/td)[9]"
                    ).get_attribute("innerHTML")
                    TmpCorpType = TmpCorpType.strip()
                    TmpAmountReal = ""
                    if TmpCorpType == "公司屬性":
                        TmpAmountReal = _myweb.find_element_by_xpath(
                            "(//table[@class='table table-striped']/tbody/tr/td)[13]"
                        ).get_attribute("innerHTML")
                        TmpAmountReal = TmpAmountReal.strip()
                    else:
                        TmpAmountReal = _myweb.find_element_by_xpath(
                            "(//table[@class='table table-striped']/tbody/tr/td)[11]"
                        ).get_attribute("innerHTML")
                        TmpAmountReal = TmpAmountReal.strip()
                    if TmpStockStatus == "僑外資":
                        TmpAmountReal = _myweb.find_element_by_xpath(
                            "(//table[@class='table table-striped']/tbody/tr/td)[15]"
                        ).get_attribute("innerHTML")
                        TmpAmountReal = TmpAmountReal.strip()
                    ...
                    TmpName = _myweb.find_element_by_xpath(
                        "//*[@id='tabBusmContent']/div/table/tbody/tr[10]/td[2]/table/tbody/tr/td[1]"
                    ).get_attribute("innerHTML")
                    TmpCorp = TmpCorp[0:TmpCorp.index("<span")].strip()
                    TmpAdd = TmpAdd[0:TmpAdd.index('<span')].strip()
                    TmpName = TmpName.strip()
                    ...
                    _worksheet.append([
                        _PageCurrent,
                        str(i), "TmpDataTypeNotFound", "TmpDataTypeNotFound",
                        "TmpDataTypeNotFound", TmpDataType,
                        str(DateTimeNow)
                    ])
                    i = i + 1
                    _myweb.back()
                    ...
                    # if isDebugMode: quit()
                    self.tableMyError.append(
                        [str(DateTimeNow), TmpDataType, _PageCurrent,
                         str(i)])
                    continue
                # quit()
                _worksheet.append([
                    _PageCurrent,
                    str(i), TmpCorp, TmpAdd, TmpName, TmpDataType,
                    str(DateTimeNow)
                ])
                # time.sleep(1)
                _myweb.back()
                ...
                try:
                    if TmpStockStatus == "僑外資":
                        ...
                        print(e)
                        self.tableMyError.append([
                            str(DateTimeNow), self.myQryCond, self.myStartPage,
                            self.myStopPage, self.myDataType,
                            'Error! MyMoreLinkCollection1', TmpDataType,
                            _PageCurrent,
                            str(i)
                        ])
                    _myweb.back()
                    ...
                    self.tableMyError.append([
                        str(DateTimeNow), self.myQryCond, self.myStartPage,
                        self.myStopPage, self.myDataType,
                        'Error! MyMoreLinkCollection2', TmpDataType,
                        _PageCurrent,
                        str(i)
                    ])
                    _myweb.back()
    ...
    def MyMoreLinkCollection2(self, _workbook, _worksheet, _myweb,
                              _PageCurrent):
        CountMoreLinkMouseOut = len(
            _myweb.find_elements_by_xpath("//*[@class='moreLinkMouseOut']"))
        # ListMoreLink = list()
    ...
    def myMainCrawler(self):
        try:
            ...
            if self.myDataType2 == "1":
                myweb.find_element_by_xpath(
                    "//input[@value='brCmpyType']").click()
            if self.myDataType3 == "1":
                myweb.find_element_by_xpath(
                    "//input[@value='busmType']").click()
            if self.myDataType4 == "1":
                myweb.find_element_by_xpath(
                    "//input[@value='factType']").click()
            if self.myDataType5 == "1":
                myweb.find_element_by_xpath(
                    "//input[@value='lmtdType']").click()
            if self.myDataType1 == "0":
                myweb.find_element_by_xpath(
                    "//input[@value='cmpyType']").click()
            ...
            try:
                ...
                print("請勿關閉程式......")
                DataRowsTotal = myweb.find_element_by_id(
                    'totalCount').get_attribute("value")
                PageTotal = myweb.find_element_by_id(
                    'totalPage').get_attribute("value")
                PageCurrent = myweb.find_element_by_id(
                    'currentPage').get_attribute("value")
            except Exception as e:
                print(e)
                print("程式正常，搜尋結果只有 1 頁.....")
                DataRowsTotal = myweb.find_element_by_xpath(
                    "(//*[@class='col-lg-12 col-md-12 col-sm-12 col-xs-12']/div)[7]"
                ).get_attribute("innerHTML")
                DataRowsTotal = DataRowsTotal[DataRowsTotal.index("共") +
                                              1:DataRowsTotal.index("筆"
                                                                    )].strip()
            ...
            ws.append([
                'qryCond', '搜尋資料', '資料種類', '登記現況', '搜尋筆數', '搜尋頁數', '程式版本',
                '網頁版本'
            ])
            ws.append([
                self.myQryCond, 'infoAddr', self.myDataType, 'isAliveY',
                DataRowsTotal, PageTotal, self.myAppVersion,
                self.officialSiteVersion
            ])  # 商工登記公示資料查詢服務 - https://goo.gl/D6onx3
            ...
            while i < int(PageTotal) + 1:
                ...
                DateTimeNow = datetime.datetime.now()
                tableMylog = [[
                    DateTimeNow, self.myAppVersion,
                    str(mySessionID), self.myQryCond, self.myStartPage,
                    self.myStopPage, self.myDataType, DataRowsTotal, PageTotal,
                    PageCurrent
                ]]
                self.MyMoreLinkCollection(wb, ws, myweb, PageCurrent)
                # MyMoreLinkCollection2(wb, ws, myweb, PageCurrent)
                # 保存
                wb.save(filename="data/myCrawler2018_" + self.myQryCond + "_" +
                        self.myDataType + "_" + str(mySessionID) + "_" +
                        str(DateTimeNowFile.strftime("%Y%m%d%H%M%S")) +
                        ".xlsx")
                with open('data/mylog_' +
                          str(DateTimeNowFile.strftime("%Y%m")) + '.csv',
                          'a',
                          newline='') as csvfile:
                    # 建立 CSV 檔寫入器
                ...
                self.tableMyError = []
                if self.myStopPage != 0 and i >= self.myStopPage:
                    break
                elif i >= 499:
            ...
            if self.isDebugMode:
                print("Start: " + str(DateTimeStart))
            if self.isDebugMode:
                print("Stop: " + str(DateTimeStop))
            if self.isDebugMode:
                print("Elapsed: " + str(DateTimeStop - DateTimeStart))
            # time.sleep(0)

            if self.isTurnOffChrome:
                myweb.quit()

        except Exception as e:
            print(
                'Something wrong, please copy the following error message, and then contact Patrick.'
            )
            print(e)
